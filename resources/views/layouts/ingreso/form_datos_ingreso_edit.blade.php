...
<div class="mb-3 col-md-2">
    ...
    <select name="id_empresa" id="id_empresa" class=" form-control" aria-label="Default select example" tabindex="1">
        @foreach ($empresas as $empresa)
            @if ($empresa->id_empresa==$ingreso->id_empresa)
...
<div class="mb-3 col-md-3">
    ...
    <select name="id_proveedor" id="id_proveedor" class="form-control select_clientes  select2" aria-label="Default select example" tabindex="2">
        @foreach ($proveedores as $proveedor)
            @if ($proveedor->id_proveedor==$ingreso->id_proveedor)
                <option selected value="{{$proveedor->id_proveedor}}">{{$proveedor->primer_nombre}} {{$proveedor->apellido_paterno}} {{$proveedor->apellido_materno}}ㅤㅤNDIP: {{$proveedor->nro_dip}}</option>
            @else
...
<div class="mb-3 col-md-2">
    ...
    <input required id="fecha_entrega" value="{{$ingreso->fecha_entrega}}" name="fecha_entrega" class="form-control" type="date"  min="1900-01-01" tabindex="5"/>
</div>   
<div class="mb-3 col-md-4">
    <label for="" class="form-label">Comentarios</label>
    <input  id="comentarios" name="comentarios" type="text" class="form-control" tabindex="3" value="{{$ingreso->comentarios}}">
</div>
...
<div class="mb-3 col-md-5">
    ...
    <select name="id_pedido" id="id_pedido" class="form-control select_clientes  select2" aria-label="Default select example" tabindex="2">
        ...
        @foreach ($pedidos as $pedido)
            @if ($pedido->id_pedido == $ingreso->id_pedido)
                <option selected value="{{$pedido->id_pedido}}">ID Pedido: &nbsp {{$pedido->id_pedido}} &nbsp &nbsp Cliente:&nbsp{{$pedido->clientes->primer_nombre}}&nbsp{{$pedido->clientes->apellido_paterno}}&nbsp{{$pedido->clientes->apellido_materno}} &nbsp &nbsp Fecha de Entrega:&nbsp{{$pedido->fecha_entrega}}</option>
            @else
...
<div class="mb-3 col-md-2">
    ...
    <input  id="num_comprobante" name="num_comprobante" type="text" class="form-control" tabindex="3" value="{{$ingreso->numero_comprobante}}">
</div>
