    ...
    <tbody>
        @foreach ($cotizaciones as $cotizacion)
            ...
            <tr>
                ...
                @if ($cotizacion->estado=='espera')
                    <td style="width: 390px">
                        <form action="{{route('cotizaciones.destroy',$cotizacion->id_cotizacion)}}" method="POST" class="formBorrar">    
                            @csrf
                            @method('DELETE')
                            <a href="{{route('pedidos.iniciarPedido',$cotizacion->id_cotizacion)}}" class="btn btn-primary">Iniciar Pedido</a>
                            <a target="_blank" href="{{route('cotizaciones.detalleReporte',$cotizacion->id_cotizacion)}}" class="btn" style="background: rgb(156, 39, 39); color: white">
